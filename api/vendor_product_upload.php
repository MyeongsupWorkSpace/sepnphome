...
function require_master_vendor_upload(): void {
...
function sanitize_name_vendor(string $name): string {
...
foreach ($_FILES as $file) {
  ...
  if ($type && !preg_match('/^image\//i', $type)) continue;
  $safe = sanitize_name_vendor($orig);
  $target = $uploadDir . DIRECTORY_SEPARATOR . (uniqid('vendor_') . '_' . $safe);
